...
const Home: NextPage = () => {
  ...
  <div className="bg-snow flex h-screen flex-col gap-4  dark:bg-dark-mode-bg transition duration-75">
    ...
    <motion.div className="w-full md:w-4/5 md:max-w-md-width gap-7 max-w-mob-search shadow-lg flex flex-col dark:bg-dark-blue rounded-2xl h-mob-container mx-auto">
      ...
      <motion.div initial={{opacity : 0}} animate={{opacity : 1}} transition={{duration : 1.5}} className="flex flex-col ml-5 mt-6 gap-4 md:grid md:grid-cols-2 border-2">
        <div className="flex flex-row gap-5">
            ...
            <p className={`font-Space-Mono ${!Info.location  ? 'text-light-grey' : 'text-navy-blue dark:text-white'}`}>{Info.location === null ? 'Not Available' : Info.location}</p>
        </div>
        <div className="flex flex-row gap-4 ">
          ...
          <a href={Info.blog} className={`font-Space-Mono ${!Info.blog  ? 'text-light-grey ' : 'text-light-grey  dark:text-white'}`}>{!Info.blog ? 'Not Available' : Info.blog}</a>
        </div>
        <div className="flex flex-row gap-4">
